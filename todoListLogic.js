...
const { secretKey, transporter, sender } = require("./config");
    ...
    try {
      ...
      if (!existingUser) {
        ...
        await transporter.sendMail({
          from: sender,
          to: email,
          subject: "[LOGIN] Login detected from your account !!",
          html: `
    <html>
    <body>
        <h1>Login Detected!</h1>
        <p>Hello,</p>
        <p>We have detected a login to your account. If this was you, you can ignore this message. If you didn't log in, please take action immediately.</p>
        <p>Thank you.</p>
    </body>
    </html>
`,
...
router.post("/api/tasks", authMiddleware, async (req, res) => {
  try {
    const { taskTitle, startDate, endDate, description, priority, status } =
      req.body;
...
router.put("/api/tasks/:taskId", authMiddleware, async (req, res) => {
  try {
    ...
    await conn.collection("demotasks").updateOne(
      { _id: taskId, userId },
      {
